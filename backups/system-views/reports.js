...
const DEFAULT_SYSTEM_VIEWS = [
  {
    name: '標準報表',
    fields: ['date', 'project', 'task', 'description', 'startTime', 'endTime', 'duration', 'hours', 'amount'],
    format: 'xlsx',
    isDefault: true,
    isSystem: true,
    description: '包含所有基本欄位的標準報表格式'
  },
  {
    name: '簡易報表',
    fields: ['date', 'project', 'task', 'hours', 'amount'],
    format: 'xlsx',
    isDefault: false,
    isSystem: true,
    description: '簡化版報表，僅包含關鍵欄位'
  }
];
...
router.get('/export', async (req, res) => {
  try {
    const { startDate, endDate, format = 'xlsx', projectIds, viewId } = req.query;
    ...
    if (exportFormat === 'xlsx') {
      return await exportExcel(actions, fields, req, res);
    } else if (exportFormat === 'csv') {
      return await exportCsv(actions, fields, req, res);
    } else {
 ...
 * 匯出 Excel 格式
 */
async function exportExcel(actions, customFields, req, res) {
  ...
  if (customFields && Array.isArray(customFields) && customFields.length > 0) {
    worksheet.columns = defaultColumns.filter(col => customFields.includes(col.key));
  } else {
 ...
 * 匯出 CSV 格式
 */
async function exportCsv(actions, customFields, req, res) {
  const workbook = new ExcelJS.Workbook();
  const worksheet = workbook.addWorksheet('時間記錄');
...
router.get('/views', async (req, res) => {
  try {
    const views = await ReportView.find().sort({ isSystem: -1, isDefault: -1, createdAt: -1 });
