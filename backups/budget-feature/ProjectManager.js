...
function ProjectManager() {
  ...
  const handleSubmit = async () => {
    try {
      ...
      setOpenDialog(false);
      setFormData({ name: '', isBillable: false, hourlyRate: 0, monthlyBudgetLimit: 0 });
  ...
  const handleEdit = (project) => {
    ...
    setFormData({
      ...
      monthlyBudgetLimit: project.monthlyBudgetLimit || 0
    });
    setOpenDialog(true);
  };
            ...
            <Grid container spacing={3}>
              ...
              <Grid item xs={12}>
                {stats.projectStats && Object.values(stats.projectStats).map((project) => {
                  ...
                  const hasBudget = fullProject && fullProject.isBillable && fullProject.monthlyBudgetLimit > 0;
                  
                  // 計算進度百分比
                  const hoursPercentage = hasBudget 
                    ? Math.min(Math.round((project.hours / (fullProject.monthlyBudgetLimit / fullProject.hourlyRate)) * 100), 100) 
                    : 0;
                  
                  const amountPercentage = hasBudget
                    ? Math.min(Math.round((project.amount / fullProject.monthlyBudgetLimit) * 100), 100)
                    : 0;
                  
                  // 確定實際百分比（用於顯示正確值）
                  const actualHoursPercentage = hasBudget 
                    ? Math.round((project.hours / (fullProject.monthlyBudgetLimit / fullProject.hourlyRate)) * 100)
                    : 0;
                  
                  const actualAmountPercentage = hasBudget
                    ? Math.round((project.amount / fullProject.monthlyBudgetLimit) * 100)
                    : 0;
                            ...
                            <Grid item xs={6}>
                              <Box sx={{ mb: 1.5 }}>
                                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                                  <Typography variant="body2">時間進度</Typography>
                                  <Tooltip 
                                    title={`${project.hours.toFixed(1)} / ${(fullProject.monthlyBudgetLimit / fullProject.hourlyRate).toFixed(1)} 小時`}
                                    arrow
                                  >
                                    <Typography 
                                      variant="body2" 
                                      color={actualHoursPercentage >= 100 ? 'error.main' : actualHoursPercentage >= 75 ? 'warning.main' : 'text.secondary'}
                                    >
                              ...
                              <Box>
                                <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                                  <Typography variant="body2">預算進度</Typography>
                                  <Tooltip 
                                    title={`$${project.amount.toFixed(2)} / $${fullProject.monthlyBudgetLimit.toFixed(2)}`}
                                    arrow
                                  >
                                    <Typography 
                                      variant="body2" 
                                      color={actualAmountPercentage >= 100 ? 'error.main' : actualAmountPercentage >= 75 ? 'warning.main' : 'text.secondary'}
                                    >
                                      {actualAmountPercentage}%
                                    </Typography>
                                  </Tooltip>
                                </Box>
                                <BorderLinearProgress variant="determinate" value={amountPercentage} />
                              </Box>
                            </Grid>
                          )}
                        </Grid>
            ...
            <Box key={project._id}>
              ...
              >
                <ListItemText
                  ...
                  secondary={
                    project.isBillable
                      ? project.monthlyBudgetLimit > 0
                          ? `計費專案 - ${project.hourlyRate} 元/小時 | 月預算上限: ${project.monthlyBudgetLimit} 元`
            ...
            {formData.isBillable && (
              <>
                ...
                <TextField
                  ...
                  onChange={handleFormChange}
                  helperText="設為 0 表示無上限"
                  InputProps={{
                    endAdornment: <Typography>元/月</Typography>
                  }}
                />
              </>
            )}
